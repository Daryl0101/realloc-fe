...
export const unregisterFCMTokenAPICall = async (token: string) => {
  ...
  try {
    res = await fetch(
      `${process.env.BASE_API_PATH}/notification/fcm/unregister`,
      {
        method: "PUT",
        headers: {
  ...
  return result.model
    ? { success: "FCM Token unregistered successfully" }
