...
export const rejectAllocationFamilyAPICall = async (id: string | null) => {
  const session = await getServerSession(options);
  var res = null;
  try {
    if (!id) return { error: "Allocation Family ID not found" };
    res = await fetch(
      `${process.env.BASE_API_PATH}/allocation/family/${id}/reject`,
      {
        method: "PATCH",
        headers: {
  ...
  return result.model
    ? { success: "Allocation rejected successfully" }
